...
def create_pmesii_ascope_crosswalk(location_of_interest_1):
    ...
    FUNCTION_SYSTEM_PROMPT = (
        ...
        "Full sentences should be used when describing PMESII Capabilities. Full sentences may also be used used when additional detail is required such as when describing Areas or Organizations if additional context is needed. "
        "**IMPORTANT: Each bullet will be formatted as follows: a one to two word main point, followed by a colon, followed by a FIVE TO TEN word description or longer as necessary to convey the important information. (ie: 'Main Point: Five to Ten word description')"
        "Ensure the response is as detailed as possible including dates along with full names of specific people, places (with LAT LON formatted in Decimal Degrees), and organizations when available. "
    ...
    USER_PROMPT = (
        ...
        "Unless otherwise prompted, the DEFAULT IS THAT EACH SUB-VARIABLE SECTION WILL CONTAIN TWO TO THREE BULLETS and will contain detailed information that describes the importance of the information in those bullets. "
        ...
        "IMPORTANT: EACH BULLET WILL CONTAIN A ONE TO TWO WORD MAIN POINT FOLLOWED BY A FIVE TO TEN WORD DESCRIPTION OR LONGER AS NEEDED TO CONVEY THE NECESSARY IMPORTANT INFORMATION such as names of districts, names and goals of political parties, key military capabilities in depth, key military/insurgent organizations and their structure, etc. "
        "Be as detailed as possible, but keep the response concise due to rate limiting. Use specific full names of people (including aliases), organizations and their leadership, locations (including Decimal Degree grids), buildings with grid coordinates, and specific military equipment at given locations when available. "
    )
