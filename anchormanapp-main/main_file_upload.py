...
# Create Road to War section
    ...
    user_input=""
    if show_text_area:
        user_input = st.text_area("Add additional requirements if desired", value="Keep the override statement and replace this sentence with your text.")

        
    if user_input != "Keep the override statement and replace this sentence with your text.":
        user_input = generic_prompt_add + user_input
    st.write("WARNING: Adding additional requirements may have unintended negative effects on the output of this product. Be as precise as possible and do not attempt to alter the output format (ie: bulletized format w/ references).")
    if st.button("Generate Road to War"):
        with st.spinner("Generating Road to War... This may take a moment."):
            ...
            # Render the HTML content with custom styling
